      ...
      function Timeline() {
        ...
        const placeTip=(key)=>{
          try{
            ...
            return {key,preview:m?m.preview:'',bm:m?m.bm:false,time:m?m.time:'',status:m?m.status:'',left,top,isLeft};
          }catch(e){ return null; }
        };
        ...
        const onRootLeave=()=>{
          ...
          hideTimerRef.current=window.setTimeout(()=>{ hideTimerRef.current=null; hideTip(); },260);
        };
        ...
        const onTipLeave=()=>{ // 移出气泡：短防抖后关闭（容忍选字划出边缘）
          if(hideTimerRef.current) window.clearTimeout(hideTimerRef.current);
          hideTimerRef.current=window.setTimeout(()=>{ hideTimerRef.current=null; hideTip(); },80);
        };
        const onKeyDown=(e)=>{
        ...
        const renderTooltip=()=>{
          ...
          return React.createElement('div',{ref:tipRef, className:'umtl-tip'+(tipVisible?' is-open':''),
              ...
              style:{left:tipData.left+'px', top:tipData.top+'px', width:'240px', minHeight:'48px', transformOrigin:tipData.isLeft?'left center':'right center',
                '--umtl-in-x':tipData.isLeft?'-7px':'7px'}}, // ★墨玉玻璃②：入场从药丸侧平移滑出（方向随气泡在导轨左/右）
            React.createElement('div',{className:'umtl-tipBody'},
              ...
              // T1.3：clamp 6 行 + T2.4：userSelect 可选中复制；★墨玉玻璃：文字用 foreground
              React.createElement('div',{style:{display:'-webkit-box',WebkitLineClamp:6,WebkitBoxOrient:'vertical',overflow:'hidden',color:'var(--dsw-alias-label-primary-foreground)',whiteSpace:'pre-wrap', wordBreak:'break-word', fontSize:'12.5px', lineHeight:'17px', userSelect:'text', WebkitUserSelect:'text', cursor:'auto'}}, tipData.preview || '（空消息）')
            )
          );
        };
          ...
          React.createElement('div',{ref:rootRef, className:cls, role:'navigation','aria-label':'User message timeline', onPointerDown:onDown, onPointerMove:onMove, onPointerUp:onUp, onPointerEnter:onRootEnter, onPointerLeave:onRootLeave, onKeyDown:onKeyDown, tabIndex:0, style:{outline:'none'}},
            ...
            React.createElement('div',{className:'umtl-markers'+(entered?' umtl-stagger':'')},
              ...
              markers.map((m,idx)=>{
                ...
                return React.createElement('div',{key:m.key,'data-key':m.key, className:c, onMouseEnter:()=>onEnter(m), onClick:()=>scrollToKey(m.key), 'aria-label':'跳至第 '+(idx+1)+' 轮', role:'button', tabIndex:-1,
