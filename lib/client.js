    ...
    function apply(ctx) {
      ...
      function markersEqual(a,b){
        ...
        for(let i=0;i<a.length;i++){
          if(a[i].key!==b[i].key||a[i].preview!==b[i].preview||a[i].bm!==b[i].bm||a[i].time!==b[i].time||a[i].status!==b[i].status||a[i].hasImage!==b[i].hasImage||a[i].isOlder!==b[i].isOlder||!imagesEqual(a[i].images,b[i].images)) return false;
        }
        return true;
      }
      ...
      function Timeline() {
        ...
        const scan=React.useCallback(()=>{
          try{
            ...
            for(let i=0;i<all.length;i++){
              ...
              out.push({key,preview,bm,time,status,hasImage,images});
            }
            const hm=isOlderBtn();
            if(hm!==hasMoreRef.current){ hasMoreRef.current=hm; setHasMore(hm); }
            if(hm){
              out.unshift({key:'__older__', isOlder:true, preview:'加载更早轮次', bm:false, time:'', status:'', hasImage:false, images:[]});
            }
        ...
        const placeTip=(key)=>{
          try{
            ...
            return {key,preview:m?m.preview:'',bm:m?m.bm:false,time:m?m.time:'',status:m?m.status:'',hasImage:m?m.hasImage:false,isOlder:m?m.isOlder:false,images:m?(m.images||[]):[],left,top,isLeft};
          }catch(e){ return null; }
        };
        ...
        const scrollToKey=async (key, attempt=0, instant=false)=>{
          try{
            ...
            // P2-C：首跳未命中 DOM 时开启目标药丸寻址脉冲（非拖拽即时滑动）
        ...
        const hudText=(key)=>{
          try{
            ...
            if(!m.preview && m.hasImage){
              ...
              el.textContent='第 '+(idx+1)+' / '+total+' 轮'+suffix+' · '+imgLabel;
              return;
            }
          ...
          return React.createElement('div',{ref:tipRef, className:'umtl-tip'+(tipVisible?' is-open':''), 'aria-live':'polite', 'aria-atomic':'true',
            ...
            React.createElement('div',{className:'umtl-tipBody'},
              ...
              (function(){
                ...
                if(tipData.preview){
                  const clamp = tipData.hasImage ? 3 : 6;
                  return React.createElement('div',{key:tipData.key, style:{display:'-webkit-box',WebkitLineClamp:clamp,WebkitBoxOrient:'vertical',overflow:'hidden',color:'var(--dsw-alias-label-primary-foreground)',whiteSpace:'pre-wrap', wordBreak:'break-word', fontSize:'12.5px', lineHeight:'17px', userSelect:'text', WebkitUserSelect:'text', cursor:'auto', animation:'umtlTextIn 80ms ease-out'}}, tipData.preview);
                }
                if(tipData.hasImage){
                  const hint = tipData.images && tipData.images.length>1 ? '[包含 ' + tipData.images.length + ' 张图片提问]' : '[纯图片提问]';
                  return React.createElement('div',{key:tipData.key, style:{display:'-webkit-box',WebkitLineClamp:3,WebkitBoxOrient:'vertical',overflow:'hidden',color:'color-mix(in srgb,var(--dsw-alias-label-primary-foreground) 60%,transparent)',whiteSpace:'pre-wrap', wordBreak:'break-word', fontSize:'12.5px', lineHeight:'17px', fontStyle:'italic', userSelect:'text', WebkitUserSelect:'text', cursor:'auto', animation:'umtlTextIn 80ms ease-out'}}, hint);
                }
                return React.createElement('div',{key:tipData.key, style:{display:'-webkit-box',WebkitLineClamp:6,WebkitBoxOrient:'vertical',overflow:'hidden',color:'var(--dsw-alias-label-primary-foreground)',whiteSpace:'pre-wrap', wordBreak:'break-word', fontSize:'12.5px', lineHeight:'17px', userSelect:'text', WebkitUserSelect:'text', cursor:'auto', animation:'umtlTextIn 80ms ease-out'}}, '（空消息）');
              })()
            )
          );
        };
          ...
          React.createElement('div',{ref:rootRef, className:cls, role:'navigation','aria-label':'User message timeline', onPointerDown:onDown, onPointerMove:onMove, onPointerUp:onUp, onPointerEnter:onRootEnter, onPointerLeave:onRootLeave, style:{outline:'none', maxHeight: rootMaxHeight ? rootMaxHeight+'px' : undefined, overflowY: rootMaxHeight ? 'auto' : undefined, maskImage: isShort ? 'none' : undefined, WebkitMaskImage: isShort ? 'none' : undefined}},
            ...
            React.createElement('div',{className:'umtl-markers'+(entered?' umtl-stagger':''), style:{gap:layout.gap+'px','--umtl-gap':layout.gap+'px'}},
              ...
              markers.map((m,idx)=>{
                ...
                const newBornClass = (!isOlder && newBornStart!==-1 && idx>=newBornStart) ? ' is-new-born' : '';
                const c='umtl-marker'+(isOlder?' is-older':'')+(isOlder && olderLoading?' is-loading':'')+(isSeeking?' is-seeking':'')+(a?' is-active':'')+(h?' is-hovered':'')+((!h && tipVisible && tipData && m.key===tipData.key)?' is-preview':'')+newBornClass;
