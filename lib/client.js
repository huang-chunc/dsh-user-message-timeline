window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      // v0.1.10：B1 首轮消失/序号错位（并集扫描+文档序+pollScan）· B2 冷启无响应（ensureScrollRoot+预热+模糊锚定）· B3 16颗上限可滚（锁死舒适尺寸，N>16可滚，不再等比收口）
      ...
      function getScrollRoot(){
        try{
          const c=document.querySelector(SEL.CONV_SCROLL) || document.querySelector('[data-conversation-scroll]') || document.querySelector('main [class*="scroll"]') || document.scrollingElement;
          if(!c) return null;
          let n=c;
      ...
      function Timeline() {
        ...
        const scan=React.useCallback(()=>{
          try{
            // B1：拓宽为并集——[data-chat-anchor-key] + HERO_ROW 内 BUBBLE + [data-chat-flow-kind=user]，按文档序合并
            const candidates=[];
            try{ document.querySelectorAll('[data-chat-anchor-key]').forEach(el=> candidates.push(el)); }catch(e){}
            try{
              const heroBubbles=document.querySelectorAll(SEL.HERO_ROW+' '+SEL.BUBBLE);
              for(let i=0;i<heroBubbles.length;i++){
                const b=heroBubbles[i];
                let anc=null;
                try{ anc=b.closest('[data-chat-anchor-key]'); }catch(e){}
                if(anc) candidates.push(anc);
                else candidates.push(b);
              }
            }catch(e){}
            try{ document.querySelectorAll('[data-chat-flow-kind="user"]').forEach(el=> candidates.push(el)); }catch(e){}
            // 按元素去重后按文档序排序
            const uniq=[];
            ...
            const running=document.querySelector(SEL.TURN_STATUS)!==null;
            for(let i=0;i<uniq.length;i++){
              const el=uniq[i];
              // 解析 key：anchor-key || turn-id || 合成
              let key=null;
              try{ key=el.getAttribute('data-chat-anchor-key'); }catch(e){}
              if(!key) try{ const anc=el.closest && el.closest('[data-chat-anchor-key]'); if(anc) key=anc.getAttribute('data-chat-anchor-key'); }catch(e){}
              if(!key) try{ key=el.getAttribute('data-turn-id') || el.getAttribute('data-chat-flow-id'); }catch(e){}
              let isSynthetic=false;
              if(!key){
                try{
                  const t=(extractText(el)||'').slice(0,24);
                  let h=0; for(let k=0;k<t.length;k++) h=(h*31 + t.charCodeAt(k))|0;
                  key='syn-'+i+'-'+(h>>>0).toString(36);
                  isSynthetic=true;
                }catch(e){ continue; }
              }
              if(seen.has(key)) continue;
              // 仅保留 user 语义（HERO 无 kind 时放行）
              if(!isSynthetic){
                let kind='';
                try{ kind=el.getAttribute('data-chat-flow-kind')||''; }catch(e){}
                if(!kind) try{ const anc=el.closest && el.closest('[data-chat-flow-kind]'); if(anc) kind=anc.getAttribute('data-chat-flow-kind')||''; }catch(e){}
                if(kind && kind!=='user'){
                  try{ if(!el.closest(SEL.HERO_ROW)) continue; }catch(e){ continue; }
                } else if(!kind){
                  let hasUser=false;
                  try{ hasUser=!!el.closest('[data-chat-flow-kind="user"]') || !!el.closest(SEL.HERO_ROW); }catch(e){}
                  if(!hasUser && !(el.getAttribute && el.getAttribute('data-chat-anchor-key'))) continue;
                }
              }
              seen.set(key,true);
              ...
              const status=running && i===uniq.length-1 ? '进行中' : '已完成';
              let hasImage=false;
        ...
        // B1：pollScan 稳定化（loadAllOlder 后等待 DOM 稳定）
        ...
        const scrollToKey=(key, attempt=0, instant=false)=>{
          try{
            ...
            if(attempt===0){
              ...
              try{ ensureScrollRoot(900).catch(()=>{}); }catch(e){}
        ...
        // B3 16颗上限可滚：N<=16 精致一屏；N>16 锁死舒适尺寸可滚，不再等比收口（density=full 兼容旧全量）
        const layout=(()=>{
          const N=markers.length;
          let density='auto';
          try{ density=window.localStorage.getItem('umtl:density')||'auto'; }catch(e){}
          if(density==='full'){
            if(N<=34) return {gap:10,mh:14,ph:4,k:1};
            const t=Math.min(1,(N-34)/46);
            let gap=10-8*t, mh=14-8*t, ph=Math.max(2,4-2*t);
            try{
              const avail=Math.min(window.innerHeight*0.7,640)-16;
          ...
          if(N<=16) return {gap:10,mh:14,ph:4,k:1};
